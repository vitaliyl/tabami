...
```bash
# Add the Tabami tap and install the Cask
...
- **macOS**: `.dmg` (Universal, Apple Silicon `aarch64`, Intel `x64`)
  > *Note for macOS direct browser downloads*: Because Tabami is free open-source software distributed without a paid Apple Developer certificate, macOS Gatekeeper may display a verification warning when opened for the first time. You can bypass this via **System Settings > Privacy & Security > Open Anyway**, or run in terminal:
  > ```bash
  > xattr -cr /Applications/Tabami.app
  > ```
  > *(Or simply install via **Homebrew Cask** above to skip this completely!)*
